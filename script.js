...
async function fetchAndCalculateDashboard() {
    try {
        ...
        showPortalToast(`Could not load dashboard data: ${error.message || "please try again."}`, "error");
    }
}
...
async function updateAllEvents(status) {
    try {
        ...
        showPortalToast(`Could not update events: ${error.message || "please try again."}`, "error");
    }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    eventForm.addEventListener("submit", async event => {
        ...
        const submitBtn = eventForm.querySelector(".submit-event-btn");
        ...
        try {
            ...
            alert(`Success! Event "${eventName}" has been added and saved.`);
        } catch (error) {
